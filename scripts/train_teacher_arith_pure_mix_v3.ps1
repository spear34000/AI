...
$trainData = Join-Path $root 'data\teacher_arith_pure_mix_v3_train.jsonl'
$evalData = Join-Path $root 'data\teacher_arith_pure_mix_v3_eval.jsonl'
$outDir = Join-Path $root ("artifacts_teacher_arith_pure_mix_fromscratch_v3_{0}" -f $RunTag)
$logDir = Join-Path $root 'logs'
$stamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$logFile = Join-Path $logDir ("arith_pure_mix_v3_{0}.log" -f $stamp)
...
if ((-not (Test-Path $trainData)) -or (-not (Test-Path $evalData))) {
    & $py scripts/build_teacher_arith_pure_mix_v3.py
    if ($LASTEXITCODE -ne 0) { throw "dataset build failed" }
}

$cmd = @(
    '-u', 'scripts/train_slm.py',
    '--data_path', 'data/teacher_arith_pure_mix_v3_train.jsonl',
    '--output_dir', $outDir,
    '--steps', '22000',
    '--batch_size', '2',
    '--grad_accum_steps', '8',
