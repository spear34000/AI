...
TRAIN_OUT = Path("data/teacher_symbolic_arith_mastery_v1_train.jsonl")
EVAL_OUT = Path("data/teacher_symbolic_arith_mastery_v1_eval.jsonl")
MANIFEST_OUT = Path("data/teacher_symbolic_arith_mastery_v1.manifest.json")
...
def make_row(prompt: str, final_answer: str, category: str, split: str) -> Dict[str, str]:
    ...
    return {
        ...
        "task_type": "korean",
        "segment_tag": "ko_teacher_symbolic_arith_mastery_v1",
        "language": "ko",
        "category": category,
        "split": split,
        "final_answer": answer,
        "answer_format": OUTPUT_FORMAT,
        "_meta_source_file": "teacher_symbolic_arith_mastery_v1",
    }


def symbolic_prompt(expr: str) -> str:
    return f"{expr}\n{LAST_LINE_ONLY}"
...
def gen_addsub(rng: random.Random, split: str, digits: int, n: int) -> List[Dict[str, str]]:
    ...
    for _ in range(n):
        ...
        rows.append(make_row(symbolic_prompt(f"{a}{op}{b}="), str(ans), f"sym_addsub_d{digits}", split))
    return rows
...
def gen_mul(rng: random.Random, split: str, a_digits: int, b_digits: int, n: int) -> List[Dict[str, str]]:
    ...
    for _ in range(n):
        ...
        rows.append(make_row(symbolic_prompt(f"{a}*{b}="), str(a * b), f"sym_mul_{a_digits}x{b_digits}", split))
    return rows
...
def gen_div(rng: random.Random, split: str, b_digits: int, q_digits: int, n: int) -> List[Dict[str, str]]:
    ...
    for _ in range(n):
        ...
        a = b * q
        rows.append(make_row(symbolic_prompt(f"{a}/{b}="), str(q), f"sym_div_{len(str(a))}by{b_digits}", split))
    return rows
...
def build_train(seed: int) -> List[Dict[str, str]]:
    ...
    rows: List[Dict[str, str]] = []
    rows.extend(gen_mul(rng, "train", 1, 1, 60000))
    rows.extend(gen_mul(rng, "train", 2, 1, 42000))
    rows.extend(gen_mul(rng, "train", 2, 2, 24000))
    rows.extend(gen_div(rng, "train", 1, 1, 24000))
    rows.extend(gen_div(rng, "train", 1, 2, 24000))
...
def main() -> None:
    train_rows = build_train(20260309)
    eval_rows = build_eval(20260310)
