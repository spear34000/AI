...
$outDir = Join-Path $root ("artifacts_teacher_arith_pure_mix_fromscratch_v2_{0}" -f $RunTag)
$logDir = Join-Path $root 'logs'
$logFile = Join-Path $logDir ("arith_pure_mix_v2_fromscratch_{0}.log" -f (Get-Date -Format 'yyyyMMdd_HHmmss'))
...
if ($StartDetached) {
    $runnerPath = Join-Path $logDir ("arith_pure_mix_v2_fromscratch_runner_{0}.ps1" -f (Get-Date -Format 'yyyyMMdd_HHmmss'))
