...
$outDir = Join-Path $root ("artifacts_teacher_arith_pure_mix_v2b_{0}" -f $RunTag)
$logDir = Join-Path $root 'logs'
$logFile = Join-Path $logDir ("arith_pure_mix_v2b_{0}.log" -f (Get-Date -Format 'yyyyMMdd_HHmmss'))
...
$trainData = Join-Path $root 'data\teacher_arith_pure_mix_v2_train.jsonl'
...
# 1. --response_loss_only : gradient ONLY on answer tokens (no instruction loss)
# 2. batch_size=2 + grad_accum=8 = same effective batch (16) but 1.3-1.5x faster
# 3. steps=25000 (fewer steps needed when loss signal is concentrated)
# 4. eval_interval=200 (more frequent monitoring)
# 5. --save_step_checkpoints for mid-training analysis
$cmd = @(
    '-u', 'scripts/train_slm.py',
    '--data_path', 'data/teacher_arith_pure_mix_v2_train.jsonl',
    '--output_dir', $outDir,
    '--steps', '25000',
    '--batch_size', '2',
    '--grad_accum_steps', '8',
...
if ($StartDetached) {
    $runnerPath = Join-Path $logDir ("arith_pure_mix_v2b_runner_{0}.ps1" -f (Get-Date -Format 'yyyyMMdd_HHmmss'))
